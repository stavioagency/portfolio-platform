 ...
 * PLAN_DEFS — no second list to keep in step. `comped` is excluded by that
 * on its own: it is not in listPlans(), because nobody can select it.
 */
export function resolvePlanCode(requested) {
  if (typeof requested !== 'string') return null;
  const code = requested.trim().toLowerCase();
  if (!code) return null;
  return getPlan(code) ? code : null;
}
