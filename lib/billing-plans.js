...
const PLAN_DEFS = [
  ...
  // ---------------------------------------------------------------------
  // TEST ONLY — never sold, never shown to a customer.
  // ---------------------------------------------------------------------
  // Exists so the full PayPal flow (approval, activation, a real charge, the
  // webhooks that follow) can be exercised for one US cent instead of $32.
  //
  // `hidden: true` keeps it out of listPlans(), which is what every
  // customer-facing surface iterates: the PlanPicker on /subscribe, the plan
  // cards in the client's Billing tab, and the marketing site (a separate
  // project that reads these numbers). Only allPlans() sees it, and only the
  // owner-gated Subscribers screen and the plan sync call that.
  //
  // The stronger guarantee is not this flag: provider_plans rows are keyed by
  // ENVIRONMENT, and activeProviderPlan() filters on PAYPAL_ENV. A sandbox row
  // is unreachable in production even if this entry were left here by mistake.
  {
    code: 'test',
    hidden: true,
    interval: 'month',
    intervalCount: 1,
    price: { SAR: 4, USD: 1 }, // 0.01 USD — the smallest charge PayPal accepts
    rank: 0,
    features: ['*'],
    name: { ar: 'اشتراك اختباري — لا تستخدمه', en: 'TEST ONLY — do not sell' },
    tagline: {
      ar: 'خطة اختبار داخلية بقيمة سنت واحد. غير معروضة للعملاء.',
      en: 'Internal one-cent test plan. Never shown to customers.',
    },
  },
];
...
// Every SELLABLE plan, cheapest tier first. This is what customer-facing
// surfaces iterate, so a hidden plan cannot be offered, priced or bought
// through any of them.
export function listPlans() {
  return PLAN_DEFS.filter((p) => !p.hidden).map(resolve).sort((a, b) => a.rank - b.rank);
}

// Every plan INCLUDING hidden ones. Two callers only: the plan sync, which has
// to mirror the whole catalogue into the provider, and the owner-gated
// Subscribers screen. Never use this to render a price to a customer.
...
// The digits alone, and the currency alone, so a caller that can render JSX can
// put the riyal MARK between them instead of the letters "ر.س". Splitting it
// out here rather than reformatting at each call site keeps the rounding rules
// in one place -- there is exactly one function in this product that decides
// what a price looks like, and this is still it.
export function amountParts(minor, currency = DISPLAY_CURRENCY) {
  ...
  return {
    digits: units === 1 || Number.isInteger(major) ? String(major) : major.toFixed(2),
    currency,
  };
}
...
export function formatAmount(minor, lang = 'ar', currency = DISPLAY_CURRENCY) {
  ...
  return lang === 'ar' ? `${parts.digits} ${label}` : `${label} ${parts.digits}`;
}
