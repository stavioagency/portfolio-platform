...
//
// Sites and Subscribers USED TO LIVE HERE, owner-only. They moved to /console
// on 2026-08-27: managing other people's accounts is a different job from
// editing one portfolio, and keeping both in this sidebar is what made the
// admin feel like two products wearing one coat. This file is now purely the
// CLIENT's navigation, and `isOwner` only decides which client-side tabs show.
//
// Home is client-only, and while ownership is still being resolved
// (isOwner === null) it does not appear. RLS remains the actual authority.
export function navGroups({ isOwner, ar, t }) {
  ...
  groups.push({
    ...
    items: [
      ...
      // Billing follows the same rule as Workspace above: for a CLIENT this is
      // "my subscription" and belongs in their settings, while an owner manages
      // every subscription from the Subscribers screen instead. Giving owners
      // this tab too would make it act on whichever workspace happens to be
      // selected, which is exactly the ambiguity that screen exists to remove.
      ...(isOwner === false ? [{ id: 'billing', icon: 'receipt', label: t('nav_billing') }] : []),
