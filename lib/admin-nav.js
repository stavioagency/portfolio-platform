...
export function navGroups({ isOwner, ar, t }) {
  ...
  if (isOwner === true) {
    groups.push({
      ...
      items: [{ id: 'clients', icon: 'users', label: ar ? 'العملاء' : 'Clients' }],
    });
  }
  ...
  groups.push({
    ...
    items: [
      // Holds the ACTIVE workspace's settings as well as its domain, since both
      // are 'this website' rather than 'my login'.
      { id: 'domains', icon: 'globe', label: ar ? 'مساحة العمل' : 'Workspace' },
