...
export function navGroups({ isOwner, ar, t }) {
  ...
  if (isOwner === true) {
    groups.push({
      ...
      items: [
        { id: 'clients', icon: 'users', label: ar ? 'المواقع' : 'Sites' },
        // Subscribers is its own screen rather than a column on Sites: it
        // answers a different question (who is paying, what is failing) and is
        // read at a different time from "who are my clients".
        { id: 'subscribers', icon: 'receipt', label: ar ? 'المشتركون' : 'Subscribers' },
      ],
    });
  }
