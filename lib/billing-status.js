...
export function deriveBilling(sub, now = Date.now()) {
  ...
  // COMPED, AND IT MAY NOW RUN OUT.
  //
  // A grant used to be permanent by construction — comped meant entitled, and
  // no date was consulted. Free access is now normally handed out for a fixed
  // stretch (30 days, renewable), so the date has to be read.
  //
  // NO END DATE STILL MEANS FOREVER, and that is the load-bearing half. Every
  // client on the platform today is comped with current_period_end null, so
  // reading a missing date as "expired" would take down all seven live
  // portfolios the moment this shipped. Absent is permanent; present is a
  // deadline. An operator choosing "forever" in the console clears the date
  // rather than setting a far-future one.
  if (sub.status === 'comped' || planCode === COMP_PLAN_CODE) {
    if (periodEnd && periodEnd <= at) {
      // The grant lapsed. This is 'expired', the same state an unpaid
      // subscription reaches, so everything downstream — the paywall, the
      // console badge, the client's own billing screen — already knows what to
      // do with it and needs no comped-specific branch.
      return view({ state: 'expired', planCode: COMP_PLAN_CODE, at, needsAction: true });
    }
    // endsAt drives daysLeft, which is what the expiry warnings count down.
    return view({ state: 'comped', planCode: COMP_PLAN_CODE, at, endsAt: periodEnd });
  }
